...
PyString_FromStringAndSize(const char *str, Py_ssize_t size)
{
    ...
    if (size == 1 && str != NULL &&
		(op = characters[*str & UCHAR_MAX]) != NULL)	// 如果创建时单个字符, 则直接从缓冲池里找
    {
#ifdef COUNT_ALLOCS
        one_strings++;
#endif
        Py_INCREF(op);			//同时把引用计数加1
        return (PyObject *)op;
    }
...
string_print(PyStringObject *op, FILE *fp, int flags)
{
    ...
    if (flags & Py_PRINT_RAW) {
		...
		printf("\naddress: %p\n", data);
		printf("refs count: %d", op->ob_refcnt);		//学习,自己加的
#endif
...
#define b ((PyStringObject *)bb)
    ...
    */
    ...
    op = (PyStringObject *)PyObject_MALLOC(PyStringObject_SIZE + size); //创建新的string对象
    if (op == NULL)
        return PyErr_NoMemory();
    PyObject_INIT_VAR(op, &PyString_Type, size);
    op->ob_shash = -1;
