...
list_resize(PyListObject *self, Py_ssize_t newsize)
{
    ...
    */
    if (allocated >= newsize && newsize >= (allocated >> 1)) {	//二分之一
...
#define PyList_MAXFREELIST 80	//缓冲池中最大数目
#endif
...
PyObject *
PyList_New(Py_ssize_t size) //size是元素个数	
{
    PyListObject *op;
    size_t nbytes;
#ifdef SHOW_ALLOC_COUNT
    ...
    nbytes = size * sizeof(PyObject *);	//list里面存放的实际上时指针
    if (numfree) {
        numfree--;
        op = free_list[numfree];
        ...
        op = PyObject_GC_New(PyListObject, &PyList_Type); //类似于malloc, 但是会有垃圾回收的处理
        if (op == NULL)
            return NULL;
#ifdef SHOW_ALLOC_COUNT
        count_alloc++;
               ...
               register PyObject *newitem)
{
    ...
    olditem = *p;
    *p = newitem;
    Py_XDECREF(olditem);	//减少原来元素的引用计数, 由于原来元素可能是NULL, 所以必须用XDECREF
    return 0;
}
...
app1(PyListObject *self, PyObject *v)
{
    ...
    if (list_resize(self, n+1) == -1)
        return -1;
	if (strcmp(v->ob_type->tp_name, "str") == 0)
	{
		MyDebugLog(((PyStringObject*)v)->ob_sval);
	}	
    Py_INCREF(v);
    PyList_SET_ITEM(self, n, v);	//设置在ob_size之后, 而不是allocated之后
    return 0;
}
 ...
 */
static int
list_ass_slice(PyListObject *a, Py_ssize_t ilow, Py_ssize_t ihigh, PyObject *v) //l[1:2] = ["a", "b"] ==> list_ass_slice(l, 1, 2, ["a", "b"])
{
