...
typedef struct {
    ...
    char *ptr;
    char *end;
    PyObject *strings; /* dict on marshal, list on unmarshal 写入pyc时, 指向一个dict; 读取pyc时, 指向list*/
    int version;
} WFILE;
...
//其实整个函数, 只会写入两种不同的数据: 整数和字符串. 对于PyObject, 会采用递归的方式, 一步步将其拆分为整数和字符串
static void
w_object(PyObject *v, WFILE *p)
{
    ...
    if (p->depth > MAX_MARSHAL_STACK_DEPTH) {
		...
		//如果是intern字符串, 在加载pyc时需要进行intern操作
        if (p->strings && PyString_CHECK_INTERNED(v)) {
			//获取v在p->strings中的序号
            PyObject *o = PyDict_GetItem(p->strings, v);	//p指向 {PyStringObject, PyIntObjec}, PyIntObject表示PyStringObject是第几个写到pyc中的intern字符串
			//intern字符串的非首次写入
            if (o) {
                long w = PyInt_AsLong(o);
                w_byte(TYPE_STRINGREF, p);					//对于非首次写入的intern字符串, 只用写入类型、第一次出现的序号即可, 这样设计是为了减少pyc的大小
                w_long(w, p);						
                goto exit;									//看到没, 这个分支没有写入字符串的内容, 详情参见pdf 157页
            }
			//intern字符串的首次写入
            else {
                int ok;
        ...
        w_byte(TYPE_TUPLE, p);	//类型标识, 用于加载pyc时判断这个字节流对应的是什么类型的数据, 两个标识之间的字节就是一个完整的数据
        n = PyTuple_Size(v);
        W_SIZE(n, p);
        for (i = 0; i < n; i++) {
    ...
    else if (PyCode_Check(v)) {	//写入PyCodeObject的代码, 可以看到, PyCodeObject中, 不同类型的数据有不同的写入方式
...
{//x就是python编译出来的PyCodeObject对象
    ...
    wf.version = version;
    w_object(x, &wf);	//实际上,是一个递归写入的过程, 会遍历所有的Code Block, 依次写入
...
r_object(RFILE *p)
{
    ...
    case TYPE_STRING:
        ...
        if (type == TYPE_INTERNED) {
            PyString_InternInPlace(&v);	//intern操作
            if (PyList_Append(p->strings, v) < 0) {	//将字符串加入rfile.strings, 后面的ref类型的数据可以直接引用这个
                retval = NULL;
                break;
            }
        }
