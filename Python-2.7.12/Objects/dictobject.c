...
lookdict(PyDictObject *mp, PyObject *key, register long hash)
{
    ...
    i = (size_t)hash & mask;	//hash值很有可能大于PyDictObject中维护的entry的个数, `与`一下一定会小于mask
    ep = &ep0[i];
    if (ep->me_key == NULL || ep->me_key == key)	//处于Unused态 或者 entry的key与待搜索的key匹配
        return ep;

    if (ep->me_key == dummy)
        freeslot = ep;
    else {
        if (ep->me_hash == hash) {	//比较两个key是否值相等
            startkey = ep->me_key;
            Py_INCREF(startkey);
